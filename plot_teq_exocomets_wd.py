...
for l in ascii.read(bpicplanets):
	(name, dist) = l
	axbpic.text(Teqbpicau(dist),0.5+0.05,name[0:2],
		rotation=90,verticalalignment='bottom',horizontalalignment='center')
...
fs = 14
for (ax,r) in zip((axsun,axbpic,axwd),(r'$D\ [R_\odot]$',r'$D\ [R_*]$',r'$D\ [R_{WD}]$')):
	ax.text(-0.01, -0.02, r"$D\ [au]$", transform=ax.transAxes,
            fontsize=fs, fontweight='bold', va='top',ha='right')

	ax.text(1.01, -0.02, r"$D\ [au]$", transform=ax.transAxes,
	...
	ax.text(1.01, 1.02, r, transform=ax.transAxes,
