...
### OLD from Sleigman comments 2024/11/04
...
axsun_x2 = axsun.secondary_xaxis('top', functions=(DeqsolRstar, DeqsolRstar))
axsun_x2.set_xlabel(r'$D\ [R_\odot]$')
...
axbpic = fig.add_axes((left,0.385,wid,0.145))
...
axwd = fig.add_axes((left,0.15,wid,0.145))
...
axwd.set_xlabel('')
#axbpic.tick_params(top=False)
axwd.axes.xaxis.set_ticklabels([])

axwd.tick_params(left=False,right=False)
axwd.set(yticklabels=[])


axwd_x2 = axwd.secondary_xaxis('bottom', functions=(Deqwdau, Teqwdau))
axwd_x2.set_xlabel(r'$D\ [au]$')

axwd.text(9000,0.8,r"WD 11145+017",fontsize=20)
...
for pp in ascii.read(wdper):
	...
	axwd.text(Teqwdau(dist.value),0.5+0.03,name[0:1],
		rotation=90,verticalalignment='bottom',horizontalalignment='center')
	axwd.scatter(Teqwdau(dist.value),0.5)
...
#	axsun.scatter(dist,0.8,color='black',alpha=0.5,zorder=-10)
	...
	axwd.text(dist,0.85,name,
		rotation=90,verticalalignment='top',
		horizontalalignment='right',alpha=0.5,zorder=-10)
	axwd.vlines(dist,0,1,colors='black',alpha=0.5,zorder=-10)
 


plt.draw()
plt.savefig("teq_exocomets_wd.pdf")
plt.show()
